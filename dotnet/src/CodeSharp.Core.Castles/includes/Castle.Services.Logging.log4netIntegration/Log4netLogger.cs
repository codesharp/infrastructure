...
namespace Castle.Services.Logging.Log4netIntegration
{
    ...
    public class Log4netLogger : MarshalByRefObject, Logger
    {
        ...
        #region Logger Members New From Castle 3.0 by wsky 201207
        ...
        {
            if (IsErrorEnabled)
                this.Error(messageFactory());
        }

        public void Fatal(Func<string> messageFactory)
        {
            if (IsFatalEnabled)
                this.Fatal(messageFactory());
        }

        public void Info(Func<string> messageFactory)
        {
            if (IsInfoEnabled)
                this.Info(messageFactory());
        }

        public void Warn(Func<string> messageFactory)
        {
            if (IsWarnEnabled)
                this.Warn(messageFactory());
        }
